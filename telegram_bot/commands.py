...
# Init frame
frames_section: Optional[FramesSection] = FramesSection(0, 0)
...
async def play(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    await update.callback_query.answer()
    global frames_section
    response = update.callback_query.data

    # Start to find.
    if response == str(PLAY):
        frames_section = FramesSection(FIRST_FRAME, LAST_FRAME)
        url_path_frame = get_url_path_frame(frames_section.get_median())
    ...
    frames_section = get_partial_frame_section(frames_section, slice_toward_right)

    # Already found it.
    if frames_section.is_mid_equal_to_limit():
        await update.effective_message.reply_text(
            f"Found! Take-off = {frames_section.get_median()}"
        )
        return EXIT
